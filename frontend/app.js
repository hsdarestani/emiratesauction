...
const card = (v, closed=false) => { const verified=v.final_price_status==='verified'; return `<article class="card ${closed?'closed':''}" onclick="openVehicle(${v.id})"><img src="${safe((v.images||[])[0]||'')}" alt="${safe(v.title)}"><div class="meta"><p>LOS #${safe(v.lot_id)} • ${closed?'BEENDET':'LIVE'}</p><h3>${safe(v.title)}</h3><div class="row"><span>${v.bid_count} Gebote<br><small class="${closed?'final':'ending'}">${closed?(verified?'Endpreis bestätigt':'Endpreis wird verifiziert'):`Endet in ${relative(v.auction_end_time)}`}</small></span><span class="right"><small>${closed?(verified?'ENDPREIS':'LETZTES LIVE-GEBOT'):'AKTUELLES GEBOT'}</small><b class="price">${money(closed?(verified?v.verified_final_price:v.last_live_bid):v.current_bid)}</b></span></div><small>Letzte Aktualisierung: ${time(v.updated_at)}</small>${closed&&verified?`<small> · Verifiziert: ${time(v.final_price_verified_at)}</small>${germanySummary(v)}`:''}${(v.condition_tags||[]).slice(0,4).map(t=>`<span class="tag">${safe(t)}</span>`).join('')}</div></article>`; };
