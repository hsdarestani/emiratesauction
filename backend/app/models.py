...
class Vehicle(Base):
    ...
    last_live_bid: Mapped[Decimal | None] = mapped_column(Numeric(14, 2))
    last_live_bid_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    ...
    monitoring_gap_seconds: Mapped[int | None] = mapped_column(Integer)
    price_data_valid: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
...
class AuctionResult(Base):
    ...
    verified_final_price: Mapped[Decimal | None] = mapped_column(Numeric(14, 2))
    final_price_verified_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    final_price_source: Mapped[str | None] = mapped_column(Text)
    final_price_status: Mapped[str] = mapped_column(String(30), default="finalizing", index=True)
